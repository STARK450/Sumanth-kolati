...
if (!apiKey) {
  return "System Error: API key missing. Configure VITE_API_KEY in Vercel.";
    ...
    // 2. Fallback: Manual extraction if .text returns object/undefined (defensive coding)
    ...
    // 3. Final validation and sanitization to prevent [object Object]
    ...
    if (text !== undefined && text !== null) {
        ...
        if (str === '[object Object]') {
             try {
                 ...
                 return "Received complex data that cannot be displayed.";
             }
        }
        return str;
    }
    ...
    try {
        if (error instanceof Error) {
            ...
            errorMessage = error;
             ...
             // Check common error properties recursively to find a string
             const deepMessage = error?.message || error?.error?.message;
             if (deepMessage) {
                 if (typeof deepMessage === 'object') {
                     errorMessage = JSON.stringify(deepMessage);
                 } else {
                     errorMessage = String(deepMessage);
                 }
             } else {
